...
void test_iterator_basic(void) {
    ...
    object item;
    while ((item = Iterator.next(it)) != NULL) {
        int *value = (int *)item;
    ...
    Assert.isNotNull(item, "Iterator reset failed");
    ...
    Assert.isNotNull(curr, "Iterator current failed");
    int *curr_value = (int *)curr;
    Assert.areEqual(&expected[0], curr_value, INT, "Iterator current mismatch");

    // Next should still return the same
    object next_item = Iterator.next(it);
    Assert.areEqual(curr, next_item, PTR, "Iterator next after current should return same");
...
__attribute__((constructor)) void init_iterator_tests(void) {
    testset("core_iterator_set", NULL, NULL);
