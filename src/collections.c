...
static iterator getIterator(object collection, ITER_TYPE type)
{
	...
	case STRB:
		...
		break;
	case SLOT:
		slot_array sa = (slot_array)collection;
		if (!sa->bucket)
			return NULL;
		start = sa->bucket;
		end = (object)sa->end;
...
/* Returns 1 if an item is found matching the predicate and returns the matching result */
static int findNext(iterator it, predicate p, object *result)
{
	if (!it || !p || !result || it->current >= it->end)
	{
		if (it && it->current < it->end)
			it->current = it->end;
		if (result)
	...
	{
		object value = (object) * (addr *)current;
		if (p(value))
		{
			it->current = (char *)current + it->step;
			*result = value;
			return 1;
		}
		current = (char *)current + it->step;
	}
...
static addr getAtIndex(addr *start, addr end, int index)
{
	if (!start)
		return 0;
	if (index < 0 || index >= countSpan(start, end))
		return 0;
	return start[index];
}
